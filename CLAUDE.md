...
- `src/` — `config.ts`, `log.ts`, `adapter/` (the chat seam: Cinderella's own domain
  ...
  Components: `names/` (CCB-S4-002), `traits/` (CCB-S4-003), `surface/` (CCB-S4-005), `bio/` (CCB-S4-006)),
...
`verify:namegen`, `verify:traits`, `verify:surface` and `verify:bio` (the profile generator; pure
computation, no DB.
