...
export async function createOrShowBotAddress(): Promise<BotAddress> {
  ...
  if (bot === null) {
    throw new RuntimeActionUnavailableError(
      'The SimpleX runtime is not running, so no contact address can be created yet. ' +
  ...
  if (profile === undefined) {
    throw new RuntimeActionUnavailableError(
      'The runtime is running but is hosting no profile, so there is no identity to ' +
        'create an address for.',
    );
  }

  // Refused rather than queued. Creating an address is an interactive network command,
  // and the core is still subscribing for about ten seconds after a restart (D-125).
  // Holding an HTTP request open for that long would look like a hung console; saying
  // so and letting the operator press again is honest and takes one sentence.
  ...
  const { simplexUserId, displayName } = profile;
