 ...
 */
const SELF_REFERENTIAL = /\{\{nameableRules\}\}/;
 ...
 */
const ELIMINATION =
  /\b(?:hidden|withheld|secret|internal|undisclosed|the rest|other)\b[^?]{0,80}\b(?:rules?|ones?|laws?)\b[^?]{0,80}\b(?:about|cover|regard|relate|concern|to do with)\b|\b(?:is|are)\s+(?:one|any|some|it)\b[^?]{0,60}\b(?:hidden|withheld|secret|internal|undisclosed)\b|\bjust\s+say\s+(?:yes|no)\b[^?]{0,40}|\byes\s+or\s+no\b/i;
...
export function asksByElimination(text: string): boolean {
  ...
  return /\b(?:hidden|withheld|secret|internal|undisclosed|rules?|laws?|not allowed|won'?t tell)\b/i.test(
    text,
  );
}
