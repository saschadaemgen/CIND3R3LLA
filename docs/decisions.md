...
**Decision 5: outgoing sends bypass `apiSendTextMessage`, and the reason is attribution.** §6.6 requires recording from the command return value, which Cinderella already does. But `apiSendMessages` **discards the `user`** the underlying `newChatItems` response carries, and `AChatItem` has no user dimension, so the typed helper's return value cannot say which profile sent. Attribution would then depend on the scheduler's own correctness. Issuing the raw command keeps `r.user.userId` on the response, so `assertSentAs` can check what the core says against what was intended: a silent misroute becomes a loud mismatch at the send site rather than a wrong row in the archive.
