...
> Note (CCB-S5-007, D-161): **nothing in this section changed**, but the SOURCE of the image did. Each hosted bot now has its own `avatar_path` (migration 049), NULL meaning the deployment default from `AVATAR_PATH`, and the boot loop loads and budgets **one image per bot** through the same `loadAvatarDataUri`. So every constant, budget and step-down below applies per bot rather than once per deployment. See architecture §32.6.
