...
| CCB-S4-004 | Multi-profile core foundation | Briefing | **Delivered on a branch** 2026-07-31 (D-096), `feature/multi-profile-core-foundation`, **not merged to main** as the briefing instructs. `npm run verify:multi-profile`, 80 checks, plus migration 023. **Scope honestly short of the briefing in two places, both recorded in D-096:** multi-profile *capture* is deferred (widening it without conversation canonicalisation would store N copies of every message with N consent derivations, on the consent path), and `startBot()` is not wired onto the runtime. **Most of §13's expectations are live-core measurements this repository cannot run**; what is proven, and what is only implemented, is listed in the harness output and the completion report |
